...
person.prototype={
    draw:function(){
        ...
        this.cobj.drawImage(this[this.status][this.state],0,0,500,400,0,0,this.width,this.height);
        this.cobj.restore();
    }
};
...
lizi1.prototype={
    draw:function(){
        this.cobj.save();
        this.cobj.translate(this.x,this.y);
        this.cobj.drawImage(this.zidanImg[this.state],0,0,100,100,0,0,this.width,this.height);
        this.cobj.restore();
    },
    update:function(){
...
game.prototype={
        ...
        for(var i=0;i<that.hinderArr.length;i++){
            ...
            if(hitPix(that.canvas,that.cobj,that.person,that.hinderArr[i])){
                if(!that.hinderArr[i].flag){
                    ...
                    xue(that.canvas,that.cobj,that.person.x+100,that.person.y);
    ...
    zhezhao:function(){
        ...
        mask.click(function(){
            ...
            that.isfir=true;
            that.zobj.x=that.person.x+140;
            that.zobj.y=that.person.y;
